...
class CityMap(FigureCanvasQTAgg):
    ...
    def init_axes(self):
        ...
        # LAad the background image
        map_image_path = 'map.jpg'  
        map_image = mpl.image.imread(map_image_path)

        # Set the background image
        self.ax.imshow(map_image, extent=[0, 1, 0, 1], aspect='auto', zorder=-1)
    ...
    def put_city(self, event):
        ...
        self.ax.plot(x_data, y_data, 'bo', markersize=10, picker=7)
    ...
    def nearest_neighbor(self, delay=0.0):
        ...
        self.current_dist = (434.78 * self.init_dist)/434.784
    ...
    def update_distance_displayed(self):
        if self.init_dist is not None:
            if self.current_dist is not None:
                adjusted_current_dist = 434.78 * self.current_dist
                self.dist_text.set_text('Coordinate Distance:\n{0:.4f}\n\n' \
                                        'Current distance\n{1:.4f}' \
                    .format(self.init_dist, adjusted_current_dist))
            else:
                self.dist_text.set_text('Coordinate Distance:\n{0:.4f}\n\n' \
                                        'Current distance\n' \
